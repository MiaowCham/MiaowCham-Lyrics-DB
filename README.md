...
- 使用可拖动宽度的注册表式目录树浏览、搜索并编辑曲目信息、平台 ID、来源；每个歌词文件是独立入口，可关联到其他曲目或拆分为独立实体；
...
- 通过类似 GitHub Desktop 的版本页查看变更、差异和历史，以及暂存、取消暂存、提交、Fetch、仅快进拉取和推送；Git 操作在后台串行执行，Windows 不显示终端窗口。
